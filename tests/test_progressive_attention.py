...
def test_real_sol_selector_runs_without_hiding_cpu_fallback(stub_lifter, installed_sol):
    original = tiny_model()
    selected = installed_sol.SolAttentionPatch().patch(original, True, .5, min_tokens=256)[0]
    _, report = run(selected)
    phases = json.loads(report)['attention']
    backend = phases['low']
...
def test_actual_kj_memory_patches_execute_full_progressive_sampling(stub_lifter, memory_nodes, kind):
    ...
    if 'sage' in kind:
        assert calls == []  # scoped adapter owns dispatch, not the original direct kernel
        assert sum(report['attention']['low']['completed_calls'].values()) > 0


def test_unknown_override_still_rejected(stub_lifter):
    model = tiny_model()
    model.model_options['transformer_options']['optimized_attention_override'] = lambda *args, **kwargs: None
    with pytest.raises(ValueError, match='unknown progressive backend'):
        run(model)
