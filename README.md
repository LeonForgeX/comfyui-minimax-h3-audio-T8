...
当前版本：**1.78.0** · 324 个节点 · GPL-3.0-or-later

本版新增 [TRT VAE 可选后端](docs/TRT_VAE_EXP.md) 和 [5 份工作流](examples/workflows/30-trt-vae)：安装检查、本机编译、仅视频解码、完整编解码，以及同潜空间双路对照。已审短片和文字对照获得整体认可，保留 EXP；长片未验证。需要独立 TensorRT 环境和本机编译引擎，旧工作流、生成模型和音频 VAE 不变。
