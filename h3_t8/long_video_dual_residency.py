...
def release_stage_residency(*components):
    ...
    for component in components:
        ...
        if not hasattr(patcher, 'model') or not hasattr(patcher, 'load_device'):
    ...
    targets = [entry for entry in loaded if id(entry.model.model) in target_ids]
    keep = [entry for entry in loaded if entry not in targets]
    ...
    freed_ram = 0
    seen = set()
    for patcher in [entry.model for entry in targets]:
    ...
    if any(callable(getattr(entry.model, 'is_dynamic', None)) and entry.model.is_dynamic()
