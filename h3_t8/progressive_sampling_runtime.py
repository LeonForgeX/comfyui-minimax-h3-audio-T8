...
def prepare_stage_conditioning(conditioning, plan, *, positive):
    ...
    if keyframes:
        ...
        resized = F.interpolate(reference[:, :, 0].float(), size=(plan.low_height // 16, plan.low_width // 16),
                                mode="bilinear", align_corners=False).to(reference.dtype).unsqueeze(2)
...
def _native_stage(model, sampler, sigmas, latent, noise, positive, negative, cfg, seed, callback):
    ...
    return comfy.samplers.sample(model, noise, positive, negative, cfg, model.load_device,
                                  ...
                                  callback=callback, disable_pbar=True, seed=seed)
...
def sample_progressive_h3(model, positive, negative, av_latent, sampler, sigmas, *,
                          ...
                          reserve_vram_mib=1024, callback=None):
